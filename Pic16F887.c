...
unsigned int8 mode=0, tt_lan=0, sobuoc=0;
unsigned int8 bdn_motor=0, motor_delay=50;
...
//!   enable_interrupts(global);
   ...
   while(true)
   {
      ...
      {
//!         lcd_gotoxy(21,1);
//!         printf(lcd_putc, "LOADCELL: %5lu", kl_loadcell);
         if(mode==1) // quay dong co
         {
            if(kl_loadcell<=khoi_luong_l1)
            {
               if(sobuoc<=23)  
               {
...
void chon_gia_tri()
{
   ...
   else if((thu_tu_cai==4)&&(tt_hien_thi==3))
   {
      ...
      tt_hien_thi=4;
      lcd_gotoxy(1,1);
      printf(lcd_putc,"T.G hien tai: %02d:%02d g", gio_ht, phut_ht);
      lcd_gotoxy(1,2);
